      ...
      <div className="container mx-auto px-4 py-3">
        <div className="flex items-center justify-between">
          ...
          <div
            ...
            onClick={() => router.push('/')}
          >
            <div className="w-11 h-11 sm:w-13 sm:h-13 rounded-full overflow-hidden border-2 border-white/30 shadow-lg shrink-0">
              <Image
                src="/logo.png"
                alt="Logo"
                width={52}
            ...
            <div className="flex flex-col">
              <h1 className="text-xl sm:text-3xl font-bold tracking-tight bg-gradient-to-r from-white to-blue-100 bg-clip-text text-transparent">
                Diamond Academy
              </h1>
              <p className="text-xs text-blue-100 opacity-80 tracking-wide hidden sm:block">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden md:flex items-center gap-3 px-4 py-2 rounded-xl bg-white/10 backdrop-blur-sm">
              <div className="w-8 h-8 rounded-full bg-gradient-to-r from-cyan-400 to-blue-500 flex items-center justify-center text-sm font-bold shrink-0">
                {user?.name?.charAt(0)?.toUpperCase() || 'U'}
              </div>
              <div className="flex flex-col">
                <span className="text-sm font-medium">{user?.name || 'User'}</span>
                <span className="text-xs text-blue-100 opacity-80">{user?.role || 'Student'}</span>
              </div>
            </div>
